...
# Function to turn off YouTube by killing the browser process
def turn_off_youtube():
    # List all processes
    for proc in psutil.process_iter(['pid', 'name']):
        try:
            # Kill all processes related to browsers that might be running YouTube
...
def execute_command(command):
    ...
    # Turn off YouTube (kill browser processes)
    elif "turn off youtube" in command:
        speak("Turning off YouTube.")
        turn_off_youtube()  # Call the function to turn off YouTube
...
def search_and_read_about(topic):
    ...
    if search_results:
        ...
        try:
            ...
            if paragraphs:
                speak(f"Here's a summary: {paragraphs[0].text}")
            else:
