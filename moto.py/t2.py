...
def get_openai_response(question):
    try:
        response = openai.Completion.create(
            model="text-davinci-003",  # or use GPT-4
            prompt=question,
            max_tokens=150
        )
...
def speak(text):
    print(text)  # Replace with actual speech synthesis if needed
...
if command:
    if "ask openai" in command or "what is" in command:
        ...
        if topic:
            # Replace with a search function if needed
...
def search_and_read_about(topic):
    speak(f"seaching about {topic}on Google.")
    search_results =list(search(topic, nun_results=5))
    if search_results:
        result_url = search_results[0]
        speak(f"i found someting on google:{result_url}")
        try:
